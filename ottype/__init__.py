...
def transform(ops1, ops2, side):
    '''Transform ops1 by ops2

    Transform ops1 to have same meaning when ops2 is applied
    to doc before ops1.

    The result of trnasform satisfies that,
    .. code::
        apply(apply(doc, ops1), trnasform(op2, op1, 'left'))
            == apply(apply(doc, ops2), trnasform(op1, op2, 'right'))
    '''

    if not check(ops1) or not check(ops2):
        raise ValueError('invalid ops')
    ...
    new_ops = []
    appender = make_appender(new_ops)
    taker, peeker = make_taker(ops1)

    for op_type, op_arg in _make_iter_ops(ops2):
        if op_type == OP_SKIP:
            length = op_arg

            while 0 < length:
                chunk = taker(length, OP_INSERT)
                appender(chunk)

                if _resolve_op(chunk)[0] != OP_INSERT:
                    length -= _op_length(chunk)
            ...
            while 0 < length:
                chunk = taker(length, OP_INSERT)
                c_op_type, c_op_arg = _resolve_op(chunk)

                if c_op_type == OP_SKIP:
                    length -= c_op_arg
                elif c_op_type == OP_DELETE:
                    length -= len(c_op_arg)
                elif c_op_type == OP_INSERT:
                    appender(chunk)

        elif op_type == OP_INSERT:
            if side == 'left' and _resolve_op(peeker())[0] == OP_INSERT:
                appender(taker(-1))
...
def compose(ops1, ops2):
    '''Compose ops1 and ops2

    The result of compose satisfies
    .. code::
        apply(apply(doc, ops1), ops2) == apply(doc, compose(ops1, ops2))
    '''
    ...
    result_ops = []
    appender = make_appender(result_ops)
    taker, _ = make_taker(ops1)
    ...
    for op_type, op_arg in _make_iter_ops(ops2):
        if op_type == OP_SKIP:
            ...
            while 0 < length:
                chunk = taker(length, OP_DELETE)
                appender(chunk)

                if _resolve_op(chunk)[0] != OP_DELETE:
                    length -= _op_length(chunk)
            ...
            while 0 < length:
                ...
                if c_op_type == OP_SKIP:
                    appender(_make_op(OP_DELETE, op_arg[offset:offset + c_op_arg]))
                    length -= c_op_arg
                    offset += c_op_arg
                elif c_op_type == OP_DELETE:
                    appender(chunk)
            ...
            appender(_make_op(OP_INSERT, op_arg))
    ...
    return trim(result_ops)


def compose_chain(*ops_list):
    result_ops = ops_list[0]
    for ops in islice(ops_list, 1, None):
        result_ops = compose(result_ops, ops)
    return result_ops
